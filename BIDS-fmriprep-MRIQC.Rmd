...
Making this its own section since I've had lots of FAQs on this topic.

See also For more details: https://fmriprep.readthedocs.io/en/latest/workflows.html#confounds-estimation
...
For more details and a simulation comparing the two strategies, see [Chris's notebook here](https://github.com/oesteban/aroma-chris-notebook/blob/master/Denoising%2Bthe%2Bdenoising.ipynb). This is a really helpful resource for understanding what they do and how they differ.

Note again that the ICA-AROMA algorithm is trained to pick up motion artifacts specifically, so won't take care of any physio-related denoising that needs to occur. For this, you may want to use the aCompCor components (along with the "Cosine" columns in the `confounds.csv` file)

_**When does the removal of components happen?  Is it part of the processing the fmriprep does when you call --use-aroma?**_
...
`fmriprep` only ever performs non-aggressive denoising (never aggressive).

_**If so, is it aggressive on non-aggressive at that point.  Or is this a separate process you do later?**_

 - there are a bunch of reasons you don’t want to do that apparently. You'd probably understand them better than I, but my general impression is that aggressive denoising means you’re removing an undesirably large amount of variance from your signal). 

Note that the ICA-AROMA algorithm is designed to specifically pick up motion artifacts and not physiological ones, and non-aggressive denoising will accordingly not do too much for stuff like CSF or arterial/venous flow. I’ve definitely seen this in GIFT when I used the denoised images - few if any motion components, but doesn’t make a difference for the physio ones that I can tell. Not sure how it handles MR artifacts like Gibbs’ ringing. I’ve seen AROMA components on the fmriprep reportlets that are clearly MR-related, just not sure if it adds those back or not. 
...
For more details: https://fmriprep.readthedocs.io/en/latest/workflows.html#confounds-estimation 27

Chris

Good question, Jess's lab just had some similar ones. I’ve been meaning to put something together on the topic based on my limited understanding since a lot of info seems to be scattered across GitHub and Neurostars forum posts, but haven’t gotten around to it yet.
